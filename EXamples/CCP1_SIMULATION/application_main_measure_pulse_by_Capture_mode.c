...
int main() {
    ...
    while (1) {
        ...
        if (CCP1_second_flag == 2) {
            ...
            pulse_period_ms = (uint_32) ( ((total_captured_value) / 1000.0));
            TIMER3_overflow = 0;
...
void CCP1_capture_ISR_APP(void) {
    ...
    if (ccp1_flag == 1) {
        ...
        CCP1_INTERRUPT_CLEAR_FLAG(); //cuz changing capture mode lead to false interrupt without clearing it will 
                                     // lead to recursive interrupt   
    } else if (ccp1_flag == 2)
 {
